...
import {
    ...
    Calculator, TrendingUp, Scale, Globe2, Code, MonitorPlay, BrainCircuit, Briefcase
} from 'lucide-react';
...
const getUnitIcon = (name: string) => {
    ...
    if (n.includes('محاسبة') || n.includes('رياضة') || n.includes('رياضيات') || n.includes('احصاء')) return <Calculator size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('اقتصاد') || n.includes('تسويق') || n.includes('استثمار')) return <TrendingUp size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('قانون') || n.includes('تشريع')) return <Scale size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('لغة') || n.includes('انجليزي') || n.includes('فرنساوي') || n.includes('english')) return <Globe2 size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('حاسب') || n.includes('تكنولوجيا') || n.includes('كمبيوتر')) return <MonitorPlay size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('برمجة') || n.includes('هياكل') || n.includes('بيانات')) return <Code size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('إدارة') || n.includes('منظمات') || n.includes('سلوك') || n.includes('اعمال')) return <Briefcase size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    if (n.includes('تفكير') || n.includes('علم نفس') || n.includes('منطق')) return <BrainCircuit size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
    return <Book size={20} className="drop-shadow-[0_0_8px_rgba(0,0,0,0.5)]" />;
};
...
export const SectionsSection: React.FC<SectionsSectionProps> = ({ user, theme, onVideoClick, onOpenFile, onDownloadFile, onUnitOpen, onUnitClose, completedExams, currentTime, onOpenRetakeModal, onLockClick, achievements, purchasedLessons, isBgAnimated, toggleBgAnimation }) => {
    const [units, setUnits] = useState<string[]>(() => DB.getSubjects(user.stage, user.year, (user as any).specialization || ''));
    const [sectionsList, setSectionsList] = useState<Content[]>(() => DB.getSections());
        ...
        <div className="w-full max-w-[600px] mx-auto px-4 py-6 space-y-4 relative">
            ...
            <div className="grid grid-cols-2 gap-3.5 py-2">
                {unitItems.map((u, i) => {
                    const updatedTime = DB.getUnitUpdatedTime(user.stage, user.year, user.semester, u.name);
                    const seenTime = unitSeenTimes[u.name] || 0;
                    const hasNewSection = updatedTime > seenTime;
                    return (
                        <button
                            key={i}
                        ...
                        >
                            ...
                            <div className={`flex-1 text-right relative min-w-0 flex flex-col pr-3 md:pr-6 items-end`}>
                                {u.items.length > 0 && hasNewSection && (
                                ...
                                <h4 className={`font-black group-hover:text-white w-full text-lg md:text-xl drop-shadow-[0_2px_12px_rgba(0,0,0,1)]`}
                                ...
                                <div className={`flex items-center gap-2 mt-1.5 opacity-60 justify-end w-full`}>
                                    ...
                                    <div className="w-1 h-1 rounded-full shrink-0" style={{ backgroundColor: theme.primary }} />
                                </div>
                            </div>
                        ...
                        <div className="flex items-center justify-between pt-8 p-4 sm:p-6 sticky top-0 z-[810] flex-row gap-4 bg-black/40 backdrop-blur-md">
                            ...
                            <div className="text-right">
                                ...
                                <div className="flex items-center justify-end gap-2 font-bold text-[10px] mt-1 uppercase tracking-widest"
                                    ...
                                    <span>{activeSection.length} سكشن</span>
